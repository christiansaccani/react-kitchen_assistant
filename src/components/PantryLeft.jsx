...
function PantryLeft({ pantryData, setPantryData }) {
    ...
    const PantryColDefs = [
      { headerName: "Item Name", field: "name", flex: .6 },
      { field: "quantity", flex: .3 },
      { field: "minimum", flex: .3 },
      { headerName: "", field: "actions", flex: .4,
        cellRenderer: (params) => {
    ...
    const handleChangePantryDatas = useCallback(() => {
      ...
      if (selectedObject) {
        if (selectedObject.name.toLowerCase() !== selectedName.toLowerCase()) {
          if (pantryData.some(item => item.name.toLowerCase() === selectedName.toLowerCase())) {
    ...
    }, [selectedName, selectedQuantity, selectedMinimum, selectedObject, pantryData]);

    const handleDeleteObject = useCallback((id) => {
      setPantryData(prevPantry => prevPantry.filter(pantry => pantry.id !== id));
    }, [pantryData]);
    ...
    return (
      <div className="PantryLeft">
        ...
        <form className="pantry-form" onSubmit={(e) => { e.preventDefault(); handleChangePantryDatas(); }} style={{width: 550}}>
